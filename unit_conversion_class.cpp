...
public:
	double value;
	double value_in_centimeter;
	string unit;
};
void convert_to_centimeter(record rec)
{
	if(rec.unit == "m")	
		rec.value_in_centimeter = rec.value*meter_to_centimeter;
	else if (rec.unit == "in")
		rec.value_in_centimeter = rec.value*inch_to_centimeter;
	else if (rec.unit == "ft")
		rec.value_in_centimeter = rec.value*inch_to_centimeter*feet_to_inch;
	else if (rec.unit == "cm")
		rec.value_in_centimeter = rec.value;
	else { 
		cout << "Error! invalid rec.unit";
		rec.value_in_centimeter = 0;
	}
}
